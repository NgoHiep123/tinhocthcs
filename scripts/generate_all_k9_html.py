...
# -*- coding: utf-8 -*-
"""
Script tự động tạo file HTML cho Khối 9 từ tất cả các file CSV
Format khối 9: K9_question_Bai_1_full.csv, q_id: K9B1_01
"""
    ...
    <div class="quiz-container w-full max-w-3xl rounded-3xl shadow-2xl p-6 md:p-10">
      <div class="text-center mb-6">
        <div class="inline-block mb-3 px-4 py-2 bg-gradient-to-r from-purple-100 to-indigo-100 rounded-full">
          <span class="text-sm font-bold text-purple-700">🎓 KHỐI 9 - BÀI {badge}</span>
        </div>
        ...
        with open(filepath, 'r', encoding='utf-8-sig') as f:  # utf-8-sig để loại bỏ BOM
            ...
            for row in reader:
                ...
                try:
                    ...
                    # Lấy mã bài từ q_id (vd: K9B1_01 -> B1, K9B10A_01 -> B10A)
...
def get_lesson_code_from_filename(filename):
    """Lấy mã bài từ tên file (vd: K9_question_Bai_1_full.csv -> B1)"""
    # Pattern: K9_question_Bai_1_full.csv -> B1
    # Hoặc: K9_question_Bai_10A_full.csv -> B10A
    match = re.search(r'Bai_(\d+)([A-Z]?)', filename)
    if match:
        num = match.group(1)
...
def get_topic_info(lesson_code):
    ...
    # Map tên bài cho khối 9 - cần cập nhật theo thực tế
    topic_names = {
        'B1': ('Máy tính và đời sống', '💻'),
        'B2': ('Mạng máy tính', '🌐'),
        'B3': ('Phần mềm máy tính', '📱'),
        'B4': ('Bảo vệ thông tin', '🔒'),
        'B5': ('Tìm kiếm thông tin', '🔍'),
        'B6': ('Trình bày thông tin', '📊'),
        'B7': ('Xử lý số liệu', '📈'),
        'B8': ('Đa phương tiện', '🎬'),
        'B9': ('Lập trình cơ bản', '💻'),
        'B10A': ('Cấu trúc rẽ nhánh', '🔀'),
        'B11': ('Vòng lặp', '🔄'),
        'B12': ('Dữ liệu và thuật toán', '📊'),
        'B13': ('Dự án Scratch', '🎮'),
        'B14': ('Tổng hợp', '🎯'),
    }
...
def main():
    """Hàm chính"""
    print("=" * 60)
    print("TAO FILE HTML CHO TIN HOC 9")
    print("=" * 60)
    
    output_dir = '../Web'  # Lưu vào thư mục Web
    csv_dir = '../Bai_tap_Tin_9'  # Đường dẫn đúng từ scripts/
    ...
    for csv_file in csv_files:
        ...
        # Nhóm theo bài học
        grouped = group_questions_by_lesson(questions)
        
        # Nếu không có lesson_code từ q_id, lấy từ tên file
        if not grouped and questions:
            lesson_code = get_lesson_code_from_filename(csv_file)
            if lesson_code:
                grouped[lesson_code] = questions
                print(f"  [INFO] Su dung ma bai tu ten file: {lesson_code}")
        
        # Gộp vào all_lessons
        for lesson_code, lesson_questions in grouped.items():
            all_lessons[lesson_code].extend(lesson_questions)
    ...
    for lesson_code, questions in sorted(all_lessons.items()):
        if questions:
            ...
            print(f"[OK] K9_{lesson_code}.html - {len(questions)} cau")
    ...
    for lesson_code, output_file, count in sorted(all_generated):
        print(f"  - K9_{lesson_code}.html: {count} cau")
    
    print(f"\n[NEXT] Buoc tiep theo:")
    print(f"  1. Mo Web/index.html de test")
    print(f"  2. Dang nhap Khoi 9 va lam thu")
