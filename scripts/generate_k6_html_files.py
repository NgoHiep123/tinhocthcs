"""
Script tự động tạo file HTML cho các bài trắc nghiệm Khối 6 - Chủ đề A
"""
...
<html lang="vi">
<head>
  <title>⚡ Trắc Nghiệm K6-{lesson_id}: {lesson_name}</title>
  <meta charset="UTF-8" />
...
def generate_html_files(csv_file, output_dir='../Web'):
    """Tạo file HTML cho mỗi bài"""
    
    print("=" * 60)
    print("TẠO FILE HTML CHO CHỦ ĐỀ A - KHỐI 6")
    print("=" * 60)
    
    # Metadata các bài
    lessons_metadata = {
        'K6A1': {'id': 'A1', 'name': 'Máy tính và ứng dụng', 'icon': '💻'},
        'K6A2': {'id': 'A2', 'name': 'Hệ điều hành và PM ứng dụng', 'icon': '💾'},
        'K6A3': {'id': 'A3', 'name': 'Tệp và thư mục', 'icon': '📁'},
        'K6A4': {'id': 'A4', 'name': 'Mạng máy tính và Internet', 'icon': '🌐'},
        'K6A5': {'id': 'A5', 'name': 'An toàn thông tin', 'icon': '🔐'},
        'K6A6': {'id': 'A6', 'name': 'Thao tác cơ bản', 'icon': '⌨️'}
    }
    
    # Đọc câu hỏi
    print(f"\n📂 Đang đọc file: {csv_file}")
    questions_by_lesson = read_questions_from_csv(csv_file)
    print(f"✅ Đã đọc {len(questions_by_lesson)} bài")
    
    # Tạo file HTML cho mỗi bài
    created_files = []
    
    for lesson_key, questions in questions_by_lesson.items():
        if lesson_key not in lessons_metadata:
            continue
        
        metadata = lessons_metadata[lesson_key]
        
        # Convert questions sang JSON
        quiz_json = '[\n'
        for i, q in enumerate(questions):
            quiz_json += '      {'
            quiz_json += f'question: "{q["question"]}", '
            quiz_json += f'options: {q["options"]}, '
            quiz_json += f'answer: {q["correct"]}'
            quiz_json += '}'
        ...
        # Lưu file
        output_file = os.path.join(output_dir, f'K6_{metadata["id"]}.html')
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(html_content)
        
        created_files.append(output_file)
        print(f"✅ Đã tạo: {output_file} ({len(questions)} câu)")
    ...
    print(f"   - Tổng câu hỏi: {sum(len(q) for q in questions_by_lesson.values())}")
    ...
    print(f"\n🚀 Bước tiếp theo:")
    print(f"   1. Mở các file HTML trong browser để test")
    print(f"   2. Cập nhật index.html để thêm link đến các bài mới")
    print(f"   3. Test đăng nhập và làm bài")
